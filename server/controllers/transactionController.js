...
export const topUpSaldo = async (req, res) => {
    ...
    if (!nominal_topup || isNaN(parseFloat(nominal_topup)) || parseFloat(nominal_topup) <= 0) {
        return res.status(400).json({ message: "Nominal top-up tidak valid atau harus lebih besar dari 0." });
    }
    ...
    if (!regionalModels) {
        return res.status(500).json({ message: `Konfigurasi database regional untuk region ID ${region_id} tidak ditemukan.` });
    }
    ...
    try {
        ...
        if (!user) {
            await t.rollback();
            return res.status(404).json({ message: "Data pengguna regional tidak ditemukan." });
        }
        ...
        try {
            ...
            console.error("Gagal sinkronisasi top-up ke GlobalTransactionLog:", syncError);
        }
        ...
        res.status(500).json({ message: "Terjadi kesalahan pada server saat top-up saldo." });
    }
};
...
export const ambilSaldo = async (req, res) => {
    ...
    if (!nominal_withdrawal || isNaN(parseFloat(nominal_withdrawal)) || parseFloat(nominal_withdrawal) <= 0) {
        return res.status(400).json({ message: "Nominal withdrawal tidak valid atau harus lebih besar dari 0." });
    }
    ...
    try {
        ...
        if (saldo_sebelum < nominal) {
            await t.rollback();
            return res.status(400).json({ message: "Saldo tidak mencukupi untuk melakukan withdrawal." });
        }
        ...
        try {
            ...
            console.error("Gagal sinkronisasi withdrawal ke GlobalTransactionLog:", syncError);
        }
        ...
        res.status(500).json({ message: "Terjadi kesalahan pada server saat ambil saldo." });
    }
};
...
export const getRiwayatTransaksi = async (req, res) => {
    ...
    try {
        ...
        if (!transactions || transactions.length === 0) {
            return res.status(200).json({ message: "Tidak ada riwayat transaksi.", data: [] });
        }
        res.status(200).json({
...
export const transferSaldo = async (req, res) => {
    ...
    const { user_id: userIdPengirim, region_id: regionIdPengirim, username: usernamePengirim } = req.user;

    if (!username_penerima || !nominal_transfer) {
        return res.status(400).json({ message: "Username penerima dan nominal transfer wajib diisi." });
    }
    ...
    try {
        ...
        const penerimaMaster = await UserMaster.findOne({ where: { username: username_penerima } });
        if (!penerimaMaster) {
            return res.status(404).json({ message: `Pengguna penerima dengan username '${username_penerima}' tidak ditemukan.` });
        }
        ...
        if (regionIdPengirim === regionIdPenerima) {
            ...
            console.log(`Memulai transfer intra-region dari ${usernamePengirim} ke ${usernamePenerima} di region ${regionIdPengirim}`);
            const regionalModels = db.getRegionalModels(regionIdPengirim);
            if (!regionalModels) {
                return res.status(500).json({ message: `Konfigurasi DB regional (ID: ${regionIdPengirim}) tidak ditemukan.` });
            }
            const { UserRegional, TransactionRegional } = regionalModels;
            ...
            try {
                ...
                if (!userPengirim) {
                    await t.rollback();
                    return res.status(404).json({ message: "Data regional pengirim tidak ditemukan." });
                }
                if (parseFloat(userPengirim.saldo) < nominal) {
                    await t.rollback();
                    return res.status(400).json({ message: "Saldo pengirim tidak mencukupi." });
                }
                ...
                try {
                    ...
                    console.error("Gagal sinkronisasi transfer intra-region ke GlobalTransactionLog:", syncError);
                }
                ...
                res.status(500).json({ message: `Gagal melakukan transfer intra-region: ${intraError.message}` });
            }

        } else {
            // --- TRANSFER INTER-REGION (Panggil SAGA Service) ---
            console.log(`Memulai transfer inter-region dari ${usernamePengirim} (Reg: ${regionIdPengirim}) ke ${usernamePenerima} (Reg: ${regionIdPenerima})`);
            try {
                const hasilSaga = await transferInterRegionSaga(
                    { userId: userIdPengirim, regionId: regionIdPengirim, username: usernamePengirim },
                    { userId: userIdPenerima, regionId: regionIdPenerima, username: usernamePenerima },
                    nominal
                );
                res.status(200).json(hasilSaga); // Hasil dari SAGA service
            } catch (sagaError) {
                console.error("Error saat transfer inter-region (SAGA):", sagaError);
                // Pesan error dari SAGA service sudah cukup deskriptif
                res.status(500).json({ message: `Transfer antar region gagal: ${sagaError.message}` });
            }
        }

    } catch (error) {
        console.error('Error utama di fungsi transferSaldo:', error);
        res.status(500).json({ message: "Terjadi kesalahan pada server saat proses transfer." });
    }
};
